  ...
  return (
    <main className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <div className="max-w-4xl mx-auto px-8 py-12">
        <h1 className="text-3xl font-bold mb-8">{t("commentsTitle")}</h1>
      ...
      {(Array.isArray(comments) ? comments : []).map((comment) => (
            <div key={comment.id} className="bg-[#111827] border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-gray-500 text-xs mb-2">
                {t("lessonLabel", { title: comment.lesson?.title })}
              </p>
                  ...
                  {comment.replies.map((reply) => (
                    <div key={reply.id} className="bg-[#0a0f1e] rounded-xl p-3">
                      <span className="text-green-400 text-sm font-semibold">
                        {t("adminReplyLabel", { name: reply.user.name })}
                      </span>
