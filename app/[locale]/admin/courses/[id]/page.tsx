...
function UserSearch({ onGrant }: { onGrant: (userId: string) => void }) {
  ...
  return (
    <div>
      ...
      {loading && <p className="text-gray-400 text-sm">{t("searching")}</p>}
...
function CompletedStudents({ courseId }: { courseId: string }) {
  ...
  return (
    <div className="bg-[#111827] border border-white/10 rounded-2xl p-6 mb-8">
      <h2 className="text-xl font-bold mb-4">{t("studentsProgress")}</h2>
      {students.length === 0 && <p className="text-gray-400 text-sm">{t("noStudents")}</p>}
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className={`text-sm font-bold ${s.completed ? "text-green-400" : "text-yellow-400"}`}>
                    {s.progress}% {s.completed ? t("completed") : t("inProgress")}
                  </p>
                </div>
                {s.certificateUrl ? (
                  ...
                  <label className="cursor-pointer bg-blue-600/20 text-blue-400 border border-blue-400/30 px-3 py-1 rounded-lg text-xs hover:bg-blue-600/30 transition-colors">
                    {uploading === s.orderId ? t("uploading") : t("uploadCertificate")}
                    <input
                      type="file"
                      accept=".pdf"
                      className="hidden"
...
export default function ManageCourse() {
  ...
  const fetchCourse = async () => {
    ...
    const data = await res.json()
    setCourse(data)
    setIsComingSoon(data.comingSoon)
...
const deleteLessonFile = async (fileId: string, lessonId: string) => {
 ...
 const grantAccess = async (userId?: string) => {
  ...
  try {
    ...
    const data = await res.json()
    setGrantSuccess(data.message || t("accessGranted"))
  } finally {
    ...
    <div className="bg-[#0d1426] border border-white/5 rounded-xl p-4">
      ...
      {editingLesson === lesson.id && (
        <div className="bg-[#111827] rounded-xl p-3 mb-3 flex flex-col gap-2">
          ...
          />
          <div className="flex gap-2">
            <button onClick={() => updateLesson(lesson.id)} className="bg-blue-600 hover:bg-blue-500 px-3 py-1 rounded-lg text-xs font-semibold">{t("save")}</button>
            <button onClick={() => setEditingLesson(null)} className="border border-white/20 px-3 py-1 rounded-lg text-xs">{t("cancel")}</button>
          </div>
        </div>
      )}
<div className="flex gap-2 mb-2">
  {lesson.videoUrl ? <span className="text-green-400 text-xs">{t("hasVideo")}</span> : <span className="text-yellow-400 text-xs">{t("noVideo")}</span>}
  {lesson.pdfUrl ? <span className="text-green-400 text-xs">{t("hasPdf")}</span> : <span className="text-gray-500 text-xs">{t("noPdf")}</span>}
...
<div className="flex gap-2">
  ...
  <label className="cursor-pointer bg-[#111827] border border-white/10 hover:border-purple-500 px-3 py-1 rounded-lg text-xs transition-colors">
    {uploadingPdf === lesson.id ? t("uploading") : t("pdfLabel")}
...
<div className="mt-3 pt-3 border-t border-white/5">
  ...
  <label className="cursor-pointer bg-[#111827] border border-white/10 hover:border-yellow-500 px-3 py-1 rounded-lg text-xs transition-colors inline-block">
    {uploadingFile === lesson.id ? t("uploading") : t("addFile")}
    <input
      type="file"
      className="hidden"
...
<div className="mb-8">
  ...
  <div className="flex flex-wrap items-center justify-between gap-4">
    <h1 className="text-3xl font-bold">{course?.title || t("loading")}</h1>
    {/* Toggle Coming Soon */}
   <button
  onClick={async () => {
    const token = localStorage.getItem("token")
...
>
  {isComingSoon ? t("comingSoonToggle") : t("activeToggle")}
</button>
  </div>
</div>
{/* Edit Course Info */}
<div className="bg-[#111827] border border-white/10 rounded-2xl p-6 mb-8">
  <h2 className="text-xl font-bold mb-4">{t("editCourseInfo")}</h2>
  <div className="flex flex-col gap-4">
    ...
    <div>
      <label className="text-sm text-gray-400 mb-2 block">{t("thumbnailLabel")}</label>
      {course?.thumbnail && (
    ...
    >
      {t("saveChanges")}
        ...
        {/* Preview Video */}
        <div className="bg-[#111827] border border-white/10 rounded-2xl p-6 mb-8">
          <h2 className="text-xl font-bold mb-2">{t("previewVideoTitle")}</h2>
          <p className="text-gray-400 text-sm mb-4">{t("previewVideoDesc")}</p>
          {course?.previewUrl && (
          ...
          <label className="cursor-pointer bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-xl text-sm font-semibold transition-colors inline-block">
            {uploadingPreview ? t("uploading") : t("uploadPreviewVideo")}
            <input
              type="file"
              accept="video/*"
              className="hidden"
            ...
            />
              ...
              {/* Grant Access */}
<div className="bg-[#111827] border border-white/10 rounded-2xl p-6 mb-8">
  <h2 className="text-xl font-bold mb-4">{t("grantFreeAccess")}</h2>
  {grantSuccess && (
        ...
        {course?.hasLevels ? (
          <div>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">{t("courseLevels")}</h2>
              <button
            ...
            {showSubCourseForm && (
              <div className="bg-[#111827] border border-blue-500/30 rounded-2xl p-6 mb-6 flex flex-col gap-4">
                <h3 className="font-bold text-blue-400">{t("newLevel")}</h3>
                <input type="text" placeholder={t("levelTitlePlaceholder")} value={subCourseForm.title} onChange={(e) => setSubCourseForm({ ...subCourseForm, title: e.target.value })} className="w-full bg-[#0a0f1e] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500" />
                <input type="text" placeholder={t("levelTypePlaceholder")} value={subCourseForm.level} onChange={(e) => setSubCourseForm({ ...subCourseForm, level: e.target.value })} className="w-full bg-[#0a0f1e] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500" />
                <textarea rows={3} placeholder={t("levelDescPlaceholder")} value={subCourseForm.description} onChange={(e) => setSubCourseForm({ ...subCourseForm, description: e.target.value })} className="w-full bg-[#0a0f1e] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 resize-none" />
                <input type="number" placeholder={t("levelPricePlaceholder", { price: course?.price })} value={subCourseForm.price} onChange={(e) => setSubCourseForm({ ...subCourseForm, price: e.target.value })} className="w-full bg-[#0a0f1e] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500" />
                <div>
                  <label className="text-sm text-gray-400 mb-2 block">{t("levelImage")}</label>
                ...
                <div className="flex gap-3">
                  <button onClick={addSubCourse} disabled={loading || !subCourseForm.title} className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 px-6 py-3 rounded-xl font-semibold transition-colors">{loading ? t("adding") : t("addLevelShort")}</button>
                  <button onClick={() => setShowSubCourseForm(false)} className="border border-white/20 px-6 py-3 rounded-xl transition-colors">{t("cancel")}</button>
                </div>
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {subCourses.length === 0 && <p className="text-gray-400 col-span-2 text-center py-8">{t("noLevelsYet")}</p>}
              {subCourses.map((sub) => (
                <div key={sub.id} className="bg-[#111827] border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all">
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-gray-400 text-sm mb-4">${sub.price} · {t("lessonsCount", { count: sub.lessons?.length || 0 })}</p>
                    <button onClick={() => router.push(`/admin/courses/${sub.id}`)} className="w-full bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-xl text-sm font-semibold transition-colors">{t("manageLevel")}</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="mb-8">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">{t("chaptersAndLessons")}</h2>
              </div>

              {/* Add Chapter */}
              <div className="bg-[#111827] border border-white/10 rounded-2xl p-6 mb-6">
                <h3 className="font-bold mb-4">{t("addChapter")}</h3>
                        ...
                        <div className="flex gap-2">
                          {editingChapter === chapter.id ? (
                            <>
                              ...
                              >{t("save")}</button>
                              <button onClick={() => setEditingChapter(null)} className="border border-white/20 px-3 py-1 rounded-lg text-xs">{t("cancel")}</button>
                            </>
                          ) : (
                            <>
                              <button
              ...
              <div className="bg-[#111827] border border-white/10 rounded-2xl p-6">
                <h3 className="font-bold mb-4">{t("addStandaloneLesson")}</h3>
                <div className="flex gap-4">
                  <input
                    type="text"
                    placeholder={t("lessonTitlePlaceholder")}
