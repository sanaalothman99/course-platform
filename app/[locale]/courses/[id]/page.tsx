...
function SubCourses({ courseId }: { courseId: string }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {subCourses.length === 0 && <p className="text-gray-400 col-span-2">{t("noLevels")}</p>}
      {subCourses.map((sub) => (
...
export default function CoursePage() {
  ...
  const fetchCourse = async () => {
    try {
      ...
      setCourse(data)
      if (data.chapters) setExpandedChapters(new Set(data.chapters.map((c: Chapter) => c.id)))
  ...
  const fetchProgress = async () => {
    ...
    if (!token) return
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/courses/${courseId}/progress`, { headers: { Authorization: `Bearer ${token}` } })
      const data = await res.json()
      if (Array.isArray(data)) setCompleted(new Set(data.map((p: any) => p.lessonId)))
    } catch { setCompleted(new Set()) }
  }

  const checkEnrollment = async () => {
    const userData = localStorage.getItem("user")
    if (userData && JSON.parse(userData).role === "ADMIN") { setEnrolled(true); return }
    const token = localStorage.getItem("token")
    if (!token) return
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/check/${courseId}`, { headers: { Authorization: `Bearer ${token}` } })
    const data = await res.json()
  ...
  const handleEnroll = async () => {
    const token = localStorage.getItem("token")
    if (!token) { router.push("/login/member"); return }
    setEnrolling(true)
    try {
  ...
  const fetchComments = async (lessonId: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/comments/lesson/${lessonId}`, { headers: { Authorization: `Bearer ${token}` } })
      const data = await res.json()
  ...
  const toggleCompleted = async (lessonId: string) => {
    ...
    if (updated.has(lessonId)) {
      ...
      if (token) await fetch(`${process.env.NEXT_PUBLIC_API_URL}/courses/progress/${lessonId}/complete`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } })
    } else {
      updated.add(lessonId)
      if (token) await fetch(`${process.env.NEXT_PUBLIC_API_URL}/courses/progress/${lessonId}/complete`, { method: "POST", headers: { Authorization: `Bearer ${token}` } })
    }
  ...
  const toggleChapter = (chapterId: string) => {
    ...
    updated.has(chapterId) ? updated.delete(chapterId) : updated.add(chapterId)
    setExpandedChapters(updated)
  }

  const getAllLessons = () => {
    if (!course) return []
    return [...(course.chapters?.flatMap(c => c.lessons) || []), ...(course.lessons || [])]
  }
  ...
  if (!course) return (
    <main className="min-h-screen bg-[#0a0f1e] text-white flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/courses" className="bg-blue-600 px-6 py-3 rounded-full">{t("backBtn")}</Link>
      </div>
    </main>
  )
        ...
        <div className="w-80 bg-[#0d1426] border-r border-white/10 flex flex-col overflow-hidden flex-shrink-0">
          <div className="bg-blue-700 p-6">
            ...
            <div className="mt-3">
              ...
              <p className="text-white/70 text-xs mt-1">{t("lessonsProgress", { completed: completedCount, total: totalLessons })}</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="max-w-4xl">
                ...
                <div className="flex flex-col gap-4">
                  {comments.length === 0 && <p className="text-gray-400 text-sm">{t("noComments")}</p>}
                        ...
                        <div className="border-l-2 border-blue-500/30 pl-4 flex flex-col gap-2">
                          {comment.replies.map((reply: any) => (
                            <div key={reply.id} className="bg-[#0a0f1e] rounded-xl p-3">
                              <span className="text-green-400 text-sm font-semibold">✅ {reply.user.name} {t("admin")}</span>
