    ...
    <main className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <section className="px-6 py-24">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-gray-400 text-center mb-16">{t("subtitle")}</p>
            ...
            {courses.map((course) => (
              <div key={course.id} className="bg-[#111827] border border-white/10 rounded-3xl overflow-hidden hover:border-blue-500/50 hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-400 text-sm leading-6 mb-4 line-clamp-2">{course.description}</p>
                  <div className="flex gap-4 text-sm text-gray-400 mb-6">
                    <span>📖 {t("lessonsCount", { count: course.lessons?.length || 0 })}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={`/courses/${course.id}`} className="bg-blue-600 hover:bg-blue-500 px-5 py-2 rounded-full text-sm font-semibold transition-colors">
                      {course.comingSoon ? t("learnMore") : t("viewCourse")}
                    </Link>
                  </div>
                </div>
              </div>
