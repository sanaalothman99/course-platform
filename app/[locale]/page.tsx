  ...
  return (
    <main className="min-h-screen bg-[#0a0f1e] text-white overflow-x-hidden">
        ...
        <div className="relative max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-16 items-center py-24">
          ...
          <div>
            ...
            <div className="mt-12">
              ...
              <p className="text-gray-500 text-xs">{t("stat3Label")}</p>
            </div>
          </div>
      ...
      <section className="px-6 py-24 bg-[#0d1426]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-blue-400 text-sm font-semibold tracking-widest uppercase">{t("coursesTitle")}</span>
            <h2 className="text-4xl md:text-5xl font-black mt-2 mb-4">{t("coursesHeading")}</h2>
              ...
              <div key={course.id} className="group bg-[#111827] border border-white/10 rounded-3xl overflow-hidden hover:border-blue-500/40 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
                ...
                <div className="p-7">
                  <div className="flex gap-2 mb-4">
                    ...
                    {course.comingSoon && (
                      <span className="text-xs text-yellow-400 bg-yellow-400/10 border border-yellow-400/20 px-3 py-1 rounded-full">🔜 {tc("comingSoon")}</span>
                    )}
                  </div>
                  ...
                  <div className="text-sm text-gray-500 mb-6">
                    📖 {course.hasLevels
                      ? `${course.children?.length || 0} ${tc("levels", { count: course.children?.length || 0 })}`
                      : `${course.lessons?.length || 0} ${tc("lessons", { count: course.lessons?.length || 0 })}`}
                  </div>

                  <div className="flex items-center justify-between">
                    {course.comingSoon ? (
                      <span className="text-yellow-400 font-bold">{tc("comingSoon")}</span>
                    ) : (
      ...
      <section className="px-6 py-24">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-blue-400 text-sm font-semibold tracking-widest uppercase">{t("whyLabel")}</span>
            <h2 className="text-4xl md:text-5xl font-black mt-2 mb-4">{t("whyTitle")}</h2>
            <p className="text-gray-400">{t("whySubtitle")}</p>
          </div>
      ...
      <section className="px-6 py-24 bg-[#0d1426]">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-black mt-2 mb-4">{t("faqHeading")}</h2>
          ...
          <div className="flex flex-col gap-3">
            {faqs.map((faq, i) => (
              <div
                ...
                className="bg-[#111827] border border-white/10 rounded-2xl overflow-hidden cursor-pointer hover:border-blue-500/30 transition-all"
              >
